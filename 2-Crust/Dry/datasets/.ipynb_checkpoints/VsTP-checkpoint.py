    ...
    with open(file_path, 'r') as file:
        ...
        for line in lines[21:]:  # Start reading from line 23
            if not line.startswith('#'):  # Skip comment lines
                parts = line.split()
                if len(parts) >= 5:  # Ensure enough columns are present
                    try:
                        T = float(parts[0])
                        P = float(parts[1])
                        rho = float(parts[2])  # Assuming rho is in the 3rd column
                        Vs = abs(float(parts[4]))  # Ensure Vs is non-negative
...
def plot(T, P, Vs, rho, title, filename, cmap):
    ...
    # Define vmin and vmax based on Vs values
    # Define vmin and vmax based on Vs values
    vmin = np.min(Vs_flat) - 0.5
    vmax = np.max(Vs_flat) + 0.5
    ...
    contour = ax1.tricontourf(triang, Vs_flat, cmap=cmap, vmin=vmin, vmax=vmax)
    cbar = plt.colorbar(contour, ax=ax1, label='Vs (Km/s)', pad=0.1)
    ax1.set_title(title)
    ax1.set_xlabel('T (°C)')
    ax1.set_ylabel('P (GPa)')
    ax1.set_ylim(0, 5)  # Setting pressure range (0-5 GPa)
    ax1.invert_yaxis()
...
# Input file name
input_file = 'HP11-dry-Arch-Basalt.tab'

# Extract base name from input file (without extension)
base_name = os.path.splitext(os.path.basename(input_file))[0]

# Read data from the input file
data_cuc = data(input_file)

# Extract columns
T_cuc, P_cuc, rho_cuc, Vs_cuc = data_cuc[:, 0], data_cuc[:, 1], data_cuc[:, 2], data_cuc[:, 3]

# Plot Vs for CUC with 'inferno' colormap and controlled range
plot(T_cuc, P_cuc, Vs_cuc, rho_cuc, f'Vs {base_name}', f'{base_name}.png', cmap_cuc)
